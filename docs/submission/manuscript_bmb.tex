...
% natbib author-year setup — BMB style (no comma between author and year)
\bibpunct{(}{)}{;}{a}{}{,}

% BMB caption format: "Fig. N" in bold, no terminal punctuation
\captionsetup[figure]{name=Fig., labelsep=space, labelfont=bf, labelformat=simple}
\captionsetup[table]{labelsep=space, labelfont=bf, labelformat=simple}

% upright derivative d
\newcommand{\diff}{\mathrm{d}}

% sn-basic.bst (Springer Nature) emits \bibcommenthead, which lives in sn-jnl.cls.
% Provide a no-op so it works under the standard article class.
\providecommand{\bibcommenthead}{}
\providecommand{\mr}[1]{}
\providecommand{\mrnumber}[1]{}
\providecommand{\zbl}[1]{}
...
\subsection{Study design, reporting, and ethics}
...
\subsection{Descent-side aperture-collapse model}
...
\subsection{Pathophysiology state machine}
...
\subsection{Muscle mechanics and multi-pathway gas exchange}
...
\subsection{Three-threshold hazard model}
...
\subsection{Descent-rate sensitivity and pathophysiology interactions}
...
\subsection{Global sensitivity and pinned-baseline stability}
...
We have extended the canonical Kanick-Doyle 2005 middle-ear-barotrauma model with a set of additions motivated by operational hypobaric-chamber needs: a continuous descent-side aperture-collapse model, a six-state URI temporal modifier, a four-state Patulous ET model, Ghadiali-FEM-inspired muscle mechanics, and Doyle 2017 multi-pathway gas exchange. The resulting simulator reproduces the pooled Colombian Aerospace Force 2010--2026 anchor cohort within 0.1 percentage points per-exposure and 0.3 percentage points career-3, transfers to two of three independent previously published cohorts within the observed Wilson 95\% CI, and remains consistent with the Kanick-Doyle Fig 3 / Groth 1986 validation under a pinned regression test. Beyond these aggregate metrics the model offers a mathematical-biology contribution that is, to our knowledge, novel: a single deterministic system of ordinary differential equations whose closed-form modifiers reproduce the chamber, airline, and PET regimes from the same parameter scaffold without retuning, and whose dominant variance driver is identifiable a priori from a Saltelli-Sobol decomposition.

\subsection{Three extensions, examined in contrast}

The continuous aperture-collapse model was the single most load-bearing addition for chamber-rate physiology. Kanick-Doyle's binary lock treatment could not simultaneously reproduce a healthy-ear 20-mmHg gradient at 300 ft$\cdot$min$^{-1}$ airline descent and a 400-mmHg gradient at 10,000 ft$\cdot$min$^{-1}$ chamber stress --- the single $R_A$ and lock threshold drifted one direction or the other, an artefact of representing a continuous biophysical process by a Heaviside step function. The Hill-function aperture with a protective free zone below 40 mmHg resolves this by leaving slow descents in the easy regime while progressively collapsing faster ones, in the same spirit that the Hill function replaces the all-or-none lac-operon switch in classical molecular biology: a smooth saturating nonlinearity is a more parsimonious description of a cooperative tissue response than two discontinuous regimes glued at a threshold. Rate-dependent tightening captures the \citet{Wang2019} rabbit histopathology gradient where pressure-change rate, not absolute altitude, predicted grade. From a dynamical-systems standpoint, the half-point $\Delta P_{1/2}$ acts as a continuous bifurcation parameter: increasing it shifts the system from "easy clearance" to "progressive failure" without the structural instability of a hard threshold, which is also why the Sobol decomposition (Section~3.4) returns it as the dominant driver of variance with negligible interaction structure.

The Patulous ET four-state model is a clinically-motivated departure from Kanick-Doyle's trivial treatment. \citet{Kanick2005} treated the patent ET as an "always open" boundary condition that drives $\Delta P$ identically to zero --- mathematically convenient, but contradicted by every recent series of patulous-ET clinical management since the Japan Otological Society criteria \citep{Kobayashi2018,Kikuchi2017}: PET ears with concurrent mucosal inflammation paradoxically close, and habitual sniffers run chronic negative middle-ear pressure with a 42.6\% type-B/C tympanogram rate \citep{Shindo2025} that is invisible to a model whose only output is a forced-zero $\Delta P$. The four-state encoding (S1 baseline, S2 inflamed, S3 sniffer, S4 post-Kobayashi-plug) restores the four qualitatively distinct biophysical regimes as four discrete steady states of the cartilaginous lumen, with S2 and S4 producing risk surfaces indistinguishable from severely stenotic obstructive disease. Paradoxical decongestant worsening in PET replaces the protective RR 0.70 with 1.4 and reflects the peritubal-soft-tissue clinical reality, supported by the null preventive effect of pseudoephedrine in the controlled-descent \citet{Moayedi2025} HBOT RCT. This level of PET granularity is not present in any prior physics-informed MEB model we have located.

The six-state URI temporal modifier is the most direct encoding of the controlled rhinovirus-challenge data \citep{McBride1989,Buchman1994,Doyle1999} into a flight-medicine simulator we are aware of. The peak-dysfunction window at days 4--7 carries a per-descent RR of 4.25, which when composed with direct ET-function effects reproduces the \citet{Lindfors2021} commercial-aircrew odds ratio of 9.02 for $\geq$3 URIs per year without overfitting --- the numerical basis for the clinical rule that aviators should not fly during URI days 1--14 and remain elevated-risk through days 15--21. From a modelling standpoint the URI table is a piecewise-constant time-varying parameter sheet that converts a static ODE into a non-autonomous one over the post-onset window; the seven-day grid was chosen to match the resolution of the \citet{Doyle2014} pressure-chamber data, not so coarse as to lose the day-4--7 peak but coarse enough to remain identifiable from the available controlled-challenge cohorts.

\subsection{Relation to prior mathematical models of middle-ear pressure regulation}

The middle ear has been described mathematically since the 1950s, and the dominant strands trace through three lineages. The first is the gas-exchange tradition initiated by Riu and refined by Doyle and colleagues over four decades, culminating in \citet{Doyle2017}'s ``formal description'' that consolidates trans-mucosal Fick diffusion of $\mathrm{O}_2$, $\mathrm{CO}_2$, $\mathrm{N}_2$, and $\mathrm{H}_2\mathrm{O}$ with active and passive ET openings into a single mass-balance system. \citet{Kanick2005} is the chamber-relevant compactification of that lineage; the Doyle 2017 reformulation is more general (it handles the pure trans-mucosal regime correctly when the ET is locked) but operationally indistinguishable from Kanick-Doyle on the descent timescales that matter for chamber training. Our extension lives inside this lineage --- it retains the Kanick-Doyle two-compartment volume, $\Delta P$ definition, and active-opening clearance --- and modifies it only in the descent-side ET aperture, the inflammatory state machine, and the parametric coupling to clinical states. The trans-mucosal exchange constants are taken directly from \citet{Doyle2011}'s species-resolved measurements; we add the trans-tympanic-membrane and trans-round-window Fick pathways from \citet{Doyle2017} (4\% and 1\% of the mucosal rate respectively) but they remain quantitatively negligible on chamber-descent timescales, as the original work itself reported.

The second lineage is the FEM and multi-scale tradition typified by \citet{Ghadiali2004}, who solved the coupled fluid-structure problem of active ET opening by the tensor veli palatini and levator veli palatini muscles. Their model resolves stress, strain, and lumen geometry as continuous fields; ours does not. We make a deliberate trade: the lumped-parameter representation of muscle priming, mucosal adhesion, and TVP/LVP timing variability captures the time-dependent qualitative behaviour (re-priming after recent swallows, adhesion buildup at high $|\Delta P|$, stochastic per-swallow efficacy) at three closed-form modifiers each rather than a partial-differential-equation system per integration step. The price is loss of spatial resolution; the gain is a simulator that runs a 5{,}000-subject synthetic cohort in seconds on a laptop --- a precondition for ABC-SMC inverse-problem inference. The two approaches are complementary: a future calibration could use Ghadiali-class simulations as the high-fidelity oracle whose summary statistics constrain our lumped multipliers.

The third lineage concerns the mastoid-volume contribution to passive buffering. \citet{Alper2011} demonstrated that mastoid volume operates as a Boyle's-law reservoir whose log-normal population distribution (median 7.0 mL, 95\% interval 2.9--16.9 mL) absorbs descent-side $\Delta P$ excursions inversely with cavity size. Our model carries this prior verbatim, and the resulting Sobol total-order index of $S_T \approx 0.005$ for mastoid volume against per-exposure $p_{\mathrm{barotitis}}$ is therefore a quantitative confirmation that the mastoid term does what \citet{Alper2011} predicted: it modulates pressure history but does not dominate it once the aperture-collapse term is in place. \citet{Alper2020} provides the most direct empirical handle on the aperture term itself --- balloon dilation paired pre- and post-measurements bracket the in vivo ET resistance --- and we identify this as the load-bearing future calibration target.

What our model adds beyond these three lineages is, first, the explicit decoupling of \emph{ascent} clearance ($\alpha = 1$, no aperture penalty) from \emph{descent} clearance (Hill-function $\alpha < 1$ with rate-tightening), which is the asymmetry chamber instructors observe at the bedside but no prior published mathematical model represents continuously; second, the inflammatory state machine that converts the static parameter set into one of $6 \times 4 = 24$ discrete clinical states (URI windows $\times$ PET states), each with its own modifier vector; and third, the cohort-aggregate calibration via ABC-SMC, which is the appropriate inverse-problem method when the available outcome data are population proportions rather than per-subject trajectories.

\subsection{Mathematical-biology framing and methodological notes}

Three methodological choices warrant explicit comment for a mathematical-biology readership. First, the choice of a Hill function (with cooperativity exponent $n = 3$) over a logistic or hyperbolic tangent is principled: the Hill family is the canonical smooth interpolant between an open and closed lumen subject to a cooperative collapse process, and the steepness exponent has a direct biophysical interpretation in terms of the number of cartilaginous cross-links that must yield simultaneously. The protective free-zone term --- $\alpha = 1$ for $|\Delta P| < 40$ mmHg --- is mathematically a soft Heaviside cutoff that prevents the Hill saturation from biting at airline-descent gradients, where active clearance is empirically intact. Second, ABC-SMC with $K = 100$ particles, four generations, and a tolerance schedule $13.9 \rightarrow 5.0$ over three summary statistics is a deliberate over-determination: with three summaries and three free hazard constants the inverse problem is exactly determined, but the SMC schedule lets the posterior contract through tolerance shrinkage without committing to a likelihood approximation \citep{Toni2009}. Third, the Saltelli-Sobol decomposition with $N = 128$ base samples and the scrambled-Sobol QMC sequence \citep{Saltelli2010} provides $S_T - S_i$ as the model's own diagnostic for missing interaction structure: in our case the gap is below 0.04, indicating that the per-exposure barotitis output is essentially additive in the four scanned parameters and that the aperture half-point is the load-bearing degree of freedom for any future empirical calibration.
...
A deeper conceptual limitation concerns the nature of the validation itself. The model is calibrated and validated against \emph{cohort-aggregate} per-exposure prevalences, not against per-subject trajectories of $\Delta P$ or per-subject barotrauma events. Calibration-in-the-large and discrimination metrics routinely reported for person-level prediction models --- the c-statistic, calibration slope, integrated calibration index --- are not applicable to a cohort-level aggregate prediction and were not computed. This is a substantive limitation: a model can fit a cohort mean correctly while mis-ranking individuals (the ecological-correlation paradox), and our external-validation pass on Morgagni 2012 and Landolfi 2009 (Section~3.2) constrains the population-level mean rather than the within-population risk gradient. The TRIPOD 2015 framework for prediction-model studies anticipates this distinction; a future per-subject study with linked baseline tympanometric and forced-response-test data would be required to validate the simulator at the individual scale and to compute the discrimination metrics that matter for personalised pre-exposure screening.
...
No per-subject machine-learning head is trained in the shipped model; the scaffolding is in place and tested on synthetic cohorts but deliberately passes through to the deterministic physics output when unfit. Ghadiali-group full-FEM ET mechanics are approximated through lumped-parameter modulators; the loss of spatial resolution is the price paid for the closed-form tractability that makes ABC-SMC and Sobol decomposition computationally feasible. Individual chamber profiles for the external-validation cohorts are not published verbatim; the simulator reconstructs the published envelope, which may differ from the exact cohort profiles in ways that affect validation numbers --- the +1.77 pp gap on Morgagni 2010 is consistent with that source of profile uncertainty, not with a structural physics error, but the analysis cannot rule out the latter without access to the original chamber-profile records.

\subsection{Incorporation of 2025--2026 evidence}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../figures/paper_c/fig_01_descent_rate_sensitivity.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../figures/paper_c/fig_02_sobol_sensitivity.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../figures/paper_c/fig_03_external_validation.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../figures/paper_c/fig_04_uri_pet_interaction.png}
